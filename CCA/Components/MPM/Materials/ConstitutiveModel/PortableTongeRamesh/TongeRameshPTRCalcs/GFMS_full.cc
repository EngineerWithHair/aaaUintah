...
double SymMatrix3::get(const int i, const int j) const{
  ...
  double val = i==j ? _values[i] : _values[6-(i+j)];	
  return val;
}
...
namespace GFMS{
  ...
  double integrateRateDependent(const double delT,
    ...
    // surface (to accommodate advection errors)
    double incPlasWork(0.0);
  ...
  // Compute stress increment 
    void Compute_Increment(GFMS::MatParamBM *BMMatParams, 
          SymMat3::SymMatrix3 sigma_c,
          double B_c,double por_c, 
          IndexParamBM *IndexParamsBM,
          SymMat3::SymMatrix3 *dsig,
          double *dB,
          SymMat3::SymMatrix3 *deps_dt,
          SymMat3::SymMatrix3 *deps_plc,
          SymMat3::SymMatrix3 *deps_elc,
          double *dporosity,
          double *dts){
  
      const double pi = 3.141592653589793;
      const double theta(BMMatParams->theta);
      const SymMat3::SymMatrix3 I_matrix(true); // Identity matrix
      double B(B_c);
      const double K(BMMatParams->bulkmodulus);
      const double G(BMMatParams->shearmodulus);
      const double Ec= BMMatParams->Ec;
      const double gamma = BMMatParams->gamma ;
      const double M_cs = BMMatParams->M_cs ;
      double dt=*dts;
           
      double p=sigma_c.trace() *(-1.0/3.0);
      SymMatrix3 sig_dev = sigma_c.deviatoric();
      double q=sig_dev.norm()*sqrt(3.0/2.0);
      double Eb= theta/((1-theta*B)*(1-theta*B))/2 *(p*p/K + q*q/(3*G));
      
      const double l_por(IndexParamsBM->por_l);
      const double u_por(IndexParamsBM->por_u);
      const double l_pori(IndexParamsBM->porind_l);
      const double u_pori(IndexParamsBM->porind_u);
              
      double por_min = l_por * pow((1.0 - B),l_pori) * exp(-B*l_pori);
      double por_max = u_por * pow((1.0 - B),u_pori) * exp(-B*u_pori);
      double tau= (por_max - por_c) / (por_max - por_min);
      double tau_cs = sqrt(Eb/Ec)*(1.0-B)/gamma ;
      
      double Ff = sqrt(Eb/Ec)*(1.0-B)- gamma*tau;
      double Md,HF;

      if (Ff < 0){
          double phi_p = pi/10 + asin(3*M_cs/(6+M_cs)) ;
          double dil_p = 6*sin(phi_p)/(3-sin(phi_p)) - M_cs ;
          Md = dil_p * gamma*(tau-tau_cs)*(tau-tau_cs);
          HF = 0.0;
       }else{
          Md = 0.0;
          HF=1.0 ;
      }
      double y =(sqrt(Eb/Ec)*(1.0-B)-gamma*tau)*sqrt(Eb/Ec)*(1.0-B)*(1.0-B) + pow(q/((M_cs + Md)*p),2.0)- 1.0 ;
      // Declare three dissipative variables
      SymMat3::SymMatrix3 deps_pl_dt  ;
      ...
      double dB_dt (0.0) ;
      double dporosity_dt(0.0) ;
        
      if (y > 0.0){
      double FB,dystar_dEb,Efi,dystar_dEfi,dEfi_dp,dystar_dq    ; 
      
      FB = sqrt(Eb/Ec)*(1.0-B)- (BMMatParams->gamma_B * tau );
       if(FB > 0.0 && std::abs(Ff) > 1.0e-8){
         dystar_dEb = 2.0*FB*(1.0-B)/sqrt(Eb*Ec) * (BMMatParams->kappa) *tau ;
       }else{
         dystar_dEb = 0 ;
       }
        Efi = -p/(1.0-por_c);
        dystar_dEfi = 2*Ff*(1.0-B)*sqrt(Eb/Ec)/Efi*(1-tau*HF);
        dEfi_dp = -1.0/(1.0-por_c) ;
        dystar_dq = 2*q/(((M_cs+Md)*p)*((M_cs+Md)*p)) ;
        SymMat3::SymMatrix3 dq_dsig=  sigma_c.deviatoric() / q * (3.0/2.0) ;
        SymMat3::SymMatrix3 dp_dsig = I_matrix * (-1.0/3.0) ;
        SymMat3::SymMatrix3 dystar_dsig = dp_dsig * dystar_dEfi * dEfi_dp + dq_dsig * dystar_dq ;
        
        deps_pl_dt = dystar_dsig/BMMatParams->eta * pow(y,BMMatParams->N_vp) ;
        dB_dt = dystar_dEb /BMMatParams->eta * pow(y,BMMatParams->N_vp) ;
        dporosity_dt = dystar_dEfi /BMMatParams->eta * pow(y,BMMatParams->N_vp) ;
      }
        SymMat3::SymMatrix3 deps_el=(*deps_dt - deps_pl_dt) * dt;
        
        // compute new stress state
        *dsig = (deps_el.isotropic()* 3.0*K*(1.0-theta*B) + deps_el.deviatoric()* 2.0*G*(1.0-theta*B) )-  sigma_c * theta  / (1.0-theta*B) *(dB_dt * dt) ;
        *dB =dB_dt * dt ;
        *dporosity = dporosity_dt * dt ;
        *deps_plc  = deps_pl_dt * dt;
        *deps_elc  = deps_el ;
  }
        
    // Compute the value of yield function for breakage model
    double Compute_Yield(GFMS::MatParamBM *BMMatParams, 
      ...
      double p=sigma_tr.trace() *(-1.0/3.0);
      SymMatrix3 sig_dev = sigma_tr.deviatoric();
      double q=sig_dev.norm()*sqrt(3.0/2.0);
      double Eb= theta/((1-theta*B)*(1-theta*B))/2 *(p*p/K + q*q/(3*G));
      ...
      double por_min = l_por * pow((1.0 - B),l_pori) * exp(-B*l_pori);
      double por_max = u_por * pow((1.0 - B),u_pori) * exp(-B*u_pori);
      double tau= (por_max - porosity) / (por_max - por_min);
      double tau_cs = sqrt(Eb/Ec)*(1.0-B)/gamma ;
